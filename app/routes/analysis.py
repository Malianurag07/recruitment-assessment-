...
def candidates(job_id: int, db: sqlite3.Connection = Depends(deps.get_db)):
    _require_job(db, job_id)
    return read_models.candidate_cards(db, job_id)


@router.get("/jobs/{job_id}/review")
def review_queue(job_id: int, db: sqlite3.Connection = Depends(deps.get_db)):
    _require_job(db, job_id)
    return read_models.needs_review(db, job_id)


@router.post("/jobs/{job_id}/rescore")
def rescore(job_id: int, db: sqlite3.Connection = Depends(deps.get_db)):
...
def export_csv(job_id: int, db: sqlite3.Connection = Depends(deps.get_db)):
    _require_job(db, job_id)
...
def export_xlsx(job_id: int, db: sqlite3.Connection = Depends(deps.get_db)):
    _require_job(db, job_id)
...
@router.get("/db")
def db_overview(db: sqlite3.Connection = Depends(deps.get_db)):
    return read_models.table_counts(db)


@router.get("/db/{table}")
def db_table(table: str, limit: int = Query(100, ge=1, le=500), db: sqlite3.Connection = Depends(deps.get_db)):
    try:
