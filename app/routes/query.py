...
def chat(job_id: int, body: Question, db: sqlite3.Connection = Depends(deps.get_db)):
...
def history(job_id: int, db: sqlite3.Connection = Depends(deps.get_db)):
...
def clear_history(job_id: int, db: sqlite3.Connection = Depends(deps.get_db)):
    with db:
